...
class MapModel {
    ...
    async generateGraph(settings, graph) {
        ...
        try {
            ...
            LOGGER.error("Failed to generate graph from osm data due to : {}", err);
        }
    ...
    async getElevations(graph) {
        LOGGER.info("Request received to getElevations");
        try {
            let answer = {
                results: [],
            };
            ...
            LOGGER.error("Failed to fetch elevations for the graph nodes due to : {}", err);
            return null;
        }
    }
}
